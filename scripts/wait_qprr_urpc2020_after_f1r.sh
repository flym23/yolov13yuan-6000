...
[[ "$RUN_ROOT" = "$ROOT"/runs/qprr_urpc2020_* && "$LAUNCHER_LOG" = "$RUN_ROOT/train/launcher.log" ]] || exit 2
...
exec "$PYTHON" -u "$ROOT/tools/run_qprr_urpc2020.py" --run-root "$RUN_ROOT" --wait >> "$LAUNCHER_LOG" 2>&1
