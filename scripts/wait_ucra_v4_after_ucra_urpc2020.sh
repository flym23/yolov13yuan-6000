...
shift 2
[[ "$RUN_ROOT" = "$ROOT"/runs/ucra_v4_* && "$LAUNCHER_LOG" = "$RUN_ROOT/train/launcher.log" ]] || exit 2
...
exec "$PYTHON" -u "$ROOT/tools/run_ucra_v4.py" --run-root "$RUN_ROOT" --wait "$@" >> "$LAUNCHER_LOG" 2>&1
