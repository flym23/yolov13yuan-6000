...
class ConfusionMatrix:
    ...
    def plot(self, normalize=True, save_dir="", names=(), on_plot=None):
        ...
        # 1. 准备类别标签：X轴和Y轴均使用「类别名 + background」（保持与原逻辑一致）
        ...
        with warnings.catch_warnings():
            ...
            seaborn.heatmap(
                ...
                square=True,
                vmin=0.0,
                # 2. 核心修改：X轴标签竖直（rotation=90），Y轴标签水平（rotation=0）
                xticklabels=ticklabels,
                yticklabels=ticklabels,
            ).set_facecolor((1, 1, 1))

        # 3. 调整X轴、Y轴标签的文字方向
        ax.tick_params(axis='x', rotation=90)  # X轴（True）标签竖直
        ax.tick_params(axis='y', rotation=0)  # Y轴（Predicted）标签水平
