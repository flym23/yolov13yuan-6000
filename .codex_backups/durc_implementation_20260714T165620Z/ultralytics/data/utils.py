    ...
    try:
        ...
        if os.path.isfile(lb_file):
            ...
            if nl := len(lb):
                ...
                # All labels
                # YOLO class ids are expected to be 0-based: [0..num_cls-1].
                # Some datasets use 1-based ids: [1..num_cls], so we allow `max_cls == num_cls`
                # only when no `0` appears in the same label file.
                max_cls = lb[:, 0].max()  # max label id
                min_cls = lb[:, 0].min()
                assert (max_cls <= num_cls - 1) or (min_cls >= 1 and max_cls == num_cls), (
                    f"Label class ids out of expected range for dataset with {num_cls} classes. "
                    f"Got min={int(min_cls)}, max={int(max_cls)}. "
                    f"Possible class labels are 0-{num_cls - 1} (0-based) or 1-{num_cls} (1-based)."
                )
...
def visualize_image_annotations(image_path, txt_path, label_map):
    ...
    with open(txt_path) as file:
        for line in file:
            ...
            h = height * img_height
            label = int(class_id)
            # Some datasets store YOLO class ids as 1-based (1..nc) while `label_map` is 0-based.
            if label not in label_map and (label - 1) in label_map:
                label -= 1
