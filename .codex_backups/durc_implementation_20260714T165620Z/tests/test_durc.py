...
from ultralytics.utils.loss import DFLoss, SUDLBboxLoss, SUDLDFLoss
...
def test_drsc_cuda_fp16_is_finite():
    ...
    x = torch.randn(2, 16, 16, 16, device="cuda", dtype=torch.float16, requires_grad=True)
    output = module(x)
...
def test_non_uniform_dfl_minimal_onnx_export(tmp_path: Path):
    ...
    torch.onnx.export(
        ...
        dynamic_axes={"logits": {0: "batch", 2: "anchors"}, "distances": {0: "batch", 2: "anchors"}},
    )
...
def test_sudl_dfl_uniform_equivalence_sigma_and_empty_shape():
    ...
    assert sudl._sigma(torch.tensor([[8.0]])).item() > sudl._sigma(torch.tensor([[64.0]])).item()
...
def test_sudl_bbox_weight_bounds_normalization_empty_and_positive_backward():
    ...
    pred_bboxes = torch.tensor(
        [[[1.1, 1.0, 3.1, 3.0], [3.0, 3.1, 5.0, 5.1], [0.0, 0.0, 1.0, 1.0], [1.0, 1.0, 2.0, 2.0]]],
        requires_grad=True,
    )
    anchor_points = torch.tensor([[2.0, 2.0], [4.0, 4.0], [0.5, 0.5], [1.5, 1.5]])
    target_bboxes = torch.tensor(
        [[[1.0, 1.0, 3.0, 3.0], [3.0, 3.0, 5.0, 5.0], [0.0, 0.0, 1.0, 1.0], [1.0, 1.0, 2.0, 2.0]]]
    )
