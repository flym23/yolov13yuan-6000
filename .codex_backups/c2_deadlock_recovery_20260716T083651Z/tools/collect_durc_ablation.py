...
def main() -> None:
    ...
    for stage in args.stages:
        rows = [
            load_seed_metrics(root, args.run_id, stage, seed) for seed in args.seeds
        ]
        all_rows.extend(rows)
        payload = stage_payload(args.run_id, stage, args.seeds, rows)
    ...
    print(
        json.dumps(
            {"csv": str(csv_path), "stages": args.stages, "seeds": args.seeds}, indent=2
        )
    )
