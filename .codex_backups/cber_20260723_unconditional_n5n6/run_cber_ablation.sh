...
payload = {
    ...
    "n5_n6_policy": "start only when N4 passes the documented L0/H3/metric/stability gate",
...
run_stage() {
  local stage=$1 failed=0 pids="" seed name weights child
  CURRENT=$stage
...
if "$PY" "$ROOT/tools/collect_cber_ablation.py" --root "$ROOT" --run-id "$RUN_ID" --stages $COMPLETED --check-n4 >"$STATE/${RUN_ID}.n4_gate.log" 2>&1; then
  write_state n4_gate_passed n4 0
else
  gate_code=$?
  if [[ "$gate_code" -eq 3 ]]; then
    CURRENT=gated
    write_state gated_no_n5_n6 n4 0
    echo "CBER N4 did not pass the documented gate; N5/N6 were intentionally not launched."
    exit 0
  fi
  exit "$gate_code"
fi
