#!/usr/bin/env bash
# GMR R1--R7 chain: five seeds per stage, with no more than three concurrent processes.
set -Eeuo pipefail
...
write_state() {
  GMR_STATE_FILE="$STATE_FILE" GMR_RUN_ID="$RUN_ID" GMR_STATUS="$1" GMR_STAGE="$CURRENT_STAGE" GMR_DETAIL="${2:-}" GMR_COMPLETED="$COMPLETED" GMR_PID="$$" "$PY" -c '
import json, os
...
payload = {
    ...
    "seed_policy": [0, 1, 2, 3, 4], "max_concurrent_processes": 3,
...
run_stage() {
  ...
  write_state training "seeds=3,4"
  run_seed_batch "$stage" train 3 4
  write_state testing "seeds=3,4"
  run_seed_batch "$stage" test 3 4
...
for stage in r1 r2 r3 r4 r5 r6 r7; do
  run_stage "$stage"
done
CURRENT_STAGE=complete
write_state complete "r1_r7_finished=true; all_stages_started_without_result_gates=true"
