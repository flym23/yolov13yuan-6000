#!/usr/bin/env bash
# CPCR L1--L4 chain: three seed processes per stage; L5 is conditional on the documented L4 gate.
set -Eeo pipefail
...
CURRENT_STAGE=l4_gate
state evaluating_l4
if "$PY" "$ROOT/tools/collect_cpcr_ablation.py" --root "$ROOT" --run-id "$RUN_ID" --stages "${COMPLETED[@]}" --check-l4 >"$STATE_DIR/$RUN_ID.l4_gate.log" 2>&1; then
  run_stage l5
  COMPLETED+=(l5)
  "$PY" "$ROOT/tools/collect_cpcr_ablation.py" --root "$ROOT" --run-id "$RUN_ID" --stages "${COMPLETED[@]}" >"$STATE_DIR/$RUN_ID.l5.collect.log" 2>&1
  CURRENT_STAGE=complete
  state complete "l5_launched=true"
  ...
  CURRENT_STAGE=complete
  state complete_l5_skipped "l4_gate_not_met"
  echo "CPCR L1--L4 completed; L5 intentionally skipped because the L4 gate was not met: run_id=$RUN_ID"
fi
