        ...
        for stage in STAGES:
            ...
            if not (run_root / "test" / stage / "all_seed_summary.json").is_file():
                ...
                for seed in (0, 1, 2):
                    ...
                    processes[seed] = (subprocess.Popen(command, stdout=handle, stderr=subprocess.STDOUT, start_new_session=True), handle)
                active_pids = [process.pid for process, _ in processes.values()]
                write_state("running", f"Training {stage} with seeds {sorted(processes)}.")
                failed = None
                while processes:
