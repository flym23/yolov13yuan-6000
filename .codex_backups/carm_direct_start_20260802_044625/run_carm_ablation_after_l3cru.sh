...
L3CRU_ROOT=/home/room305/ZZF/yolov13-6000
L3CRU_STATE_FILE=${1:-}
WAIT_LOG="$ROOT/runs/carm_ablation/after_l3cru_wait.log"

mkdir -p "$(dirname "$WAIT_LOG")"
if [[ -z "$L3CRU_STATE_FILE" ]]; then
  L3CRU_STATE_FILE=$(find "$L3CRU_ROOT/runs/train" -maxdepth 1 -type f -name 'l3cru_*.state.json' \
    -printf '%T@:%p\n' | sort -nr | head -n 1 | cut -d: -f2-)
fi
[[ -x "$PY" && -f "$L3CRU_STATE_FILE" ]] || { echo "L3-CRU dependency or Python runtime is unavailable" >&2; exit 78; }
echo "$(date -u +%FT%TZ) CARM dependency: $L3CRU_STATE_FILE" >>"$WAIT_LOG"
while true; do
