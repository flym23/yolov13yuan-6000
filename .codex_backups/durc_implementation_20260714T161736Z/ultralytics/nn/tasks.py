...
from ultralytics.nn.modules import (
    ...
    FullPAD_Tunnel,
    DSC3k2,
    DRSC,  # noqa: F401 - exported into this module for YAML/global resolution compatibility
    DRSCDSC3k2,
    GSDSC3k2,
    GSConv,
    UCRA_DetailUp,
        ...
        if not self.is_fused():
            for m in self.model.modules():
                if hasattr(m, "switch_to_deploy"):
                    m.switch_to_deploy()
                if isinstance(m, (Conv, Conv2, DWConv, GSConv)) and hasattr(m, "bn"):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
            ...
            if m in {
                ...
                n = 1
            if m in {C3k2, DSC3k2, DRSCDSC3k2, GSDSC3k2}:  # for P/U sizes
                legacy = False
                if scale in "lx":
                    args[3] = True
            if m is A2C2f:
            ...
            if m in {Detect, HRCTDetect, SUDLDetect, Segment, Pose, OBB}:
                m.legacy = legacy
            ...
            c2 = ch[f[0]]
        elif m in {UCRA_SemUp, UCRA_DetailUp}:
            if not isinstance(f, list) or len(f) != 2:
                raise ValueError(f"{m.__name__} expects from=[deep, skip], got {f}.")
            c_deep, c_skip = ch[f[0]], ch[f[1]]
            c2 = args[2]
            if c2 != nc:
                c2 = make_divisible(min(c2, max_channels) * width, 8)
            reduction = args[3] if len(args) > 3 else 4
            k = args[4] if len(args) > 4 else 3
            hidden = max(make_divisible(c2 / reduction, 8), 8)
            args = [c_deep, c_skip, c2, hidden, k]
        else:
            c2 = ch[f]
