...
class GSConv(nn.Module):
    """Pretrain-friendly Group Shuffle Convolution used by Slim-neck/AGREE-YOLO."""

    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, d=1, act=True):
        super().__init__()
        if c2 % 2:
            raise ValueError(f"GSConv requires even output channels, got {c2}.")
        c_ = c2 // 2
        self.conv = nn.Conv2d(c1, c2, k, s, autopad(k, p, d), groups=g, dilation=d, bias=False)
        self.bn = nn.BatchNorm2d(c2)
        self.act = Conv.default_act if act is True else act if isinstance(act, nn.Module) else nn.Identity()
        self.gs_cv1 = Conv(c1, c_, k, s, p, g, d, act)
        self.gs_cv2 = Conv(c_, c_, 3, 1, None, c_, d, act)
        self.gs_alpha = nn.Parameter(torch.tensor(0.15))

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        base = self.act(self.bn(self.conv(x)))
        x1 = self.gs_cv1(x)
        gs = channel_shuffle(torch.cat((x1, self.gs_cv2(x1)), dim=1), groups=2)
        return base + self.gs_alpha * gs

    def forward_fuse(self, x: torch.Tensor) -> torch.Tensor:
        base = self.act(self.conv(x))
        x1 = self.gs_cv1(x)
        gs = channel_shuffle(torch.cat((x1, self.gs_cv2(x1)), dim=1), groups=2)
        return base + self.gs_alpha * gs
...
class GSBottleneck(nn.Module):
    """Bottleneck with GSConv for neck feature fusion (AGREE-YOLO / Slim-neck)."""

    def __init__(self, c1, c2, shortcut=True, e=0.5, k1=3, k2=3):
        super().__init__()
        c_ = int(c2 * e)
        self.cv1 = GSConv(c1, c_, k1, 1)
        self.cv2 = GSConv(c_, c2, k2, 1)
        self.add = shortcut and c1 == c2
...
class GSDSC3k2(C2f):
    """DSC3k2 variant where internal DSBottlenecks are replaced with GSConv-based GSBottleneck."""

    def __init__(self, c1, c2, n=1, shortcut=True, e=0.5, k1=3, k2=3):
        super().__init__(c1, c2, n, shortcut, g=1, e=e)
        c_ = int(c2 * e)
        self.m = nn.ModuleList(GSBottleneck(c_, c_, shortcut, e=1.0, k1=k1, k2=k2) for _ in range(n))
