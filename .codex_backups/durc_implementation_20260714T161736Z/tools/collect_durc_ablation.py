...
def durc_row(root, run_id, stage, seed, complexity):
    ...
    row.update({label: float(summary["metrics"][key]) for label, key in METRIC_KEYS.items()})
...
def main():
    ...
    payload = {
        ...
        "units": {key: "fraction" for key in (*METRIC_KEYS, *SCALE_KEYS)} | {"Params": "count", "GFLOPs": "GFLOPs@640"},
