...
from ultralytics.utils.torch_utils import get_flops, get_num_gradients, get_num_params, intersect_dicts  # noqa: E402
...
def scaled_alias(config_path: Path, scale: str) -> Path:
    ...
    return config_path.with_name(config_path.name.replace("yolov13-", f"yolov13{scale}-", 1))
...
def assert_transfer(source_state, target_model, prefixes):
    ...
    for prefix in prefixes:
        ...
        if not matched:
            raise RuntimeError(f"no pretrained parameters transferred for prefix {prefix}")
    return len(transferred)


def measure_complexity(model):
    return len(list(model.modules())), get_num_params(model), get_num_gradients(model), get_flops(model, imgsz=640)
...
def build_loss_smoke(model, device, imgsz):
    ...
    batch = {
        ...
        "bboxes": torch.tensor([[0.50, 0.50, 0.20, 0.20], [0.35, 0.40, 0.12, 0.16]], device=device),
    }
    ...
    if not gradients or not all(torch.isfinite(gradient).all() for gradient in gradients):
        raise RuntimeError("SUDL synthetic training loss produced missing or non-finite gradients")
...
def main():
    ...
    original = YOLO(str(scaled_alias(cfg_dir / "yolov13.yaml", "n"))).model
    if original.stride.tolist() != [8.0, 16.0, 32.0]:
        raise RuntimeError(f"original YOLOv13 stride regression: {original.stride.tolist()}")
    if args.skip_complexity:
        baseline_complexity = {"layers": None, "params": None, "gradients": None, "gflops_640": None}
    else:
    ...
    for stage, filename in MAIN_FILES.items():
        ...
        for scale in "nslx":
            ...
            if model.stride.tolist() != [8.0, 16.0, 32.0]:
                raise RuntimeError(f"{stage}/{scale} invalid strides: {model.stride.tolist()}")
            if scale == "n":
                model = model.to(device).eval()
                sample = torch.randn(args.batch, 3, args.imgsz, args.imgsz, device=device)
                with torch.no_grad():
                    output = model(sample)
    ...
    for stage, filename in MODEL_FILES.items():
        ...
        transfer_prefixes = ["model.2.cv1.", "model.2.cv2.", "model.4.cv1.", "model.4.cv2."]
        if stage.startswith("B"):
            transfer_prefixes.extend(("model.32.cv2.", "model.32.cv3.", "model.32.dfl."))
