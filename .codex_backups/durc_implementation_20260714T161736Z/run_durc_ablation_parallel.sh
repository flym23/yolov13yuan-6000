...
wait_for_gpu_idle() {
  local gpu_pids sibling_pids
  while true; do
    sibling_pids=$(pgrep -f '/home/rtx6000/ZZF/yolov13-6000/(run_dcra_ablation_parallel.sh|tools/train_dcra_worker.py|test.py|audit_dcra_checkpoint.py)' || true)
    if [[ -n "$sibling_pids" ]]; then
      CURRENT=waiting_for_existing_chain
      write_state waiting_for_existing_chain "$CURRENT"
      sleep 60
      continue
    fi
    if ! gpu_pids=$(nvidia-smi --query-compute-apps=pid --format=csv,noheader,nounits); then
      echo "nvidia-smi compute-process query failed" >&2
      return 1
    fi
    gpu_pids=${gpu_pids//[[:space:]]/}
    [[ -z "$gpu_pids" ]] && return 0
    CURRENT=waiting_for_other_gpu_work
    write_state waiting_for_gpu "$CURRENT"
    sleep 60
  done
}
...
CURRENT=waiting_for_gpu
write_state waiting_for_gpu "$CURRENT"
wait_for_gpu_idle

CURRENT=preflight_pytest
write_state running "$CURRENT"
CUDA_VISIBLE_DEVICES=0 "$PY" -m pytest "$ROOT/tests/test_durc.py" -q \
  > "$STATE/preflight_pytest_${RUN_ID}.log" 2>&1
