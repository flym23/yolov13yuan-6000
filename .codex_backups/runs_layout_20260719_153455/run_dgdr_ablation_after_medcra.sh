#!/usr/bin/env bash
# Run the original YOLOv13 baseline then DGDR G1..G7 after the ME-DCRA M0--M7 launcher finishes successfully.
set -Eeuo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PY="${PYTHON_BIN:-/home/room305/.conda/envs/yolov13/bin/python}"
DATA="${DGDR_DATA:-/home/room305/ZZF/URPC2020half/data.yaml}"
...
PREDECESSOR_STATE="${MEDCRA_STATE:-$PREDECESSOR_ROOT/runs/urpc2020_medcra_m0_m7/state.json}"
...
CODE=0
PID="$$"
ALL_STAGES=(baseline g1 g2 g3 g4 g5 g6 g7)
START_STAGE="${DGDR_START_STAGE:-baseline}"
START_INDEX=-1
for index in "${!ALL_STAGES[@]}"; do
  if [[ "${ALL_STAGES[$index]}" == "$START_STAGE" ]]; then
    START_INDEX="$index"
    break
  fi
done
if ((START_INDEX < 0)); then
  echo "unknown DGDR_START_STAGE: $START_STAGE" >&2
  exit 64
fi
STAGES=("${ALL_STAGES[@]:START_INDEX}")
STAGE_ORDER_CSV="$(IFS=,; echo "${STAGES[*]}")"
COMPLETED=()
CACHE_READY=false
...
write_state() {
  ...
  DGDR_RUN_ID="$RUN_ID" DGDR_STATUS="$STATUS" DGDR_STAGE="$CURRENT" DGDR_CODE="$CODE" DGDR_PID="$PID" \
    DGDR_STATE="$STATE" DGDR_DATA="$DATA" DGDR_PREDECESSOR="$PREDECESSOR_STATE" DGDR_STAGE_ORDER="$STAGE_ORDER_CSV" DGDR_START_STAGE="$START_STAGE" "$PY" -c '
import json, os
...
d = {
    ...
    "stage_order": os.environ["DGDR_STAGE_ORDER"].split(","),
    "start_stage": os.environ["DGDR_START_STAGE"],
...
prepare_dataset_caches() {
  # The three seed processes share one YOLO cache path.  Build it only through
  # seed-0, then allow the remaining seeds to read the completed cache.
