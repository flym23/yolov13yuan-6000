...
payload = {
    ...
    "policy": "start CBER N4-N6 after SAMR-LCER-DCRA is no longer running; accept complete or failed terminal state",
...
while true; do
  ...
  case "$samr_status" in
    complete|failed)
      write_wait_state "samr_terminal_${samr_status}_starting_cber"
