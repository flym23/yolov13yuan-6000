...
class v8DetectionLoss:
    ...
    def __call__(self, preds, batch):
        ...
        if self.sudl_enabled and self.sudl_quality_calibration:
            with torch.no_grad():
                probability = pred_distri.detach().view(batch_size, -1, 4, self.reg_max).float().softmax(dim=-1)
        ...
        if fg_mask.sum():
            ...
            if self.sudl_enabled:
                loss[0], loss[2] = self.bbox_loss(*bbox_args, stride_tensor=stride_tensor, imgsz=imgsz)
            else:
                loss[0], loss[2] = self.bbox_loss(*bbox_args)
