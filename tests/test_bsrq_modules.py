...
def test_reliability_adaptive_fullpad_identity_and_bounds() -> None:
    ...
    factor = 1.0 + module.max_modulation * torch.tanh(module.modulator(module._cues(original, enhanced)))
    bounded_factor = factor.detach()
    assert float(bounded_factor.min()) >= 0.8 - 1e-5 and float(bounded_factor.max()) <= 1.2 + 1e-5
...
def test_scq_quality_fusion_identity_bounds_and_backward() -> None:
    ...
    bounded_reliability = reliability.detach()
    assert float(bounded_reliability.min()) >= 0.85 - 1e-5 and float(bounded_reliability.max()) <= 1.15 + 1e-5
