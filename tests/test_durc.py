...
from test import metric_names_from_data
from tools.collect_durc_ablation import (
    load_seed_metrics,
    parse_seed_overrides,
    stage_payload,
)
...
try:
    ...
    pytest.skip("DURC-only loss tests are obsolete after restoring original YOLOv13 loss.py.", allow_module_level=True)
...
def test_durc_private_dataset_maps_one_based_labels_without_mutating_source(
    ...
    cache_path = target_root / "labels" / "train.cache"
    cache_path.write_text("stale", encoding="utf-8")
    build_durc_dataset(source_data, target_root, target_data)
    assert not cache_path.exists()
    with pytest.raises(ValueError, match="outside"):
        normalize_label_line("0 0.5 0.5 0.2 0.2", source_label, 1)


def test_metric_names_require_complete_zero_based_dataset_mapping():
    names = metric_names_from_data(
        {"names": {"0": "a", 1: "b", 2: "c", 3: "d"}}, expected_nc=4
    )
    assert names == {0: "a", 1: "b", 2: "c", 3: "d"}
    with pytest.raises(ValueError, match="zero-based"):
        metric_names_from_data(
            {"names": {1: "a", 2: "b", 3: "c", 4: "d"}}, expected_nc=4
        )


def test_durc_stage_summary_uses_dcra_schema_and_percent_metrics(tmp_path: Path):
    test_dir = tmp_path / "runs" / "test" / "durc_run_a1_seed0"
    test_dir.mkdir(parents=True)
    (test_dir / "summary_metrics.json").write_text(
        """{
          "metrics": {
            "metrics/precision(B)": 0.81,
            "metrics/recall(B)": 0.72,
            "metrics/mAP50(B)": 0.73,
            "metrics/mAP75(B)": 0.64,
            "metrics/mAP50-95(B)": 0.51
          },
          "scale_metrics_percent": {"APS": 12.0, "APM": 34.0, "APL": 56.0}
        }""",
        encoding="utf-8",
    )
    row = load_seed_metrics(tmp_path, "run", "A1", 0)
    payload = stage_payload("run", "A1", [0], [row])
    ...
    assert payload["rows_percent"][0]["P"] == 81.0
    assert payload["mean"]["mAP50-95"] == 51.0
    assert payload["std"]["APS"] == 0.0
    assert payload["detail"]["APL"] == {
        "mean": 56.0,
        "std": 0.0,
        "values": [56.0],
        "best": 56.0,
    ...
    assert parse_seed_overrides("C2=1,2;C3=0,1,2") == {
        "C2": [1, 2],
        "C3": [0, 1, 2],
    }
    with pytest.raises(ValueError, match="unique"):
        parse_seed_overrides("C2=1,1")
