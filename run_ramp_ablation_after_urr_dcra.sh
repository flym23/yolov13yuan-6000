#!/usr/bin/env bash
# Start RAMP as soon as the verified URR-DCRA launcher has exited.
set -Ee -o pipefail
...
while dependency_alive; do
  echo "$(date -u +%FT%TZ) waiting for active URR-DCRA recovery"
  sleep 60
done

last_status=$("$PY" -c 'import json, sys; print(json.load(open(sys.argv[1], encoding="utf-8")).get("status", "unknown"))' "$DEPENDENCY_STATE" 2>/dev/null || echo unavailable)
echo "$(date -u +%FT%TZ) URR-DCRA launcher exited (last state=$last_status); starting RAMP K1--K4"
