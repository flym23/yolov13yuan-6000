#!/usr/bin/env python3
"""One QPRR full-URPC2020 training or evaluation worker."""
...
def main() -> None:
    ...
    if (model.model.model[-1].__class__.__name__ != 'Detect' or
            model.model.yaml.get('qprr', {}).get('enabled') is not True or
            float(model.model.yaml['qprr'].get('gain', 0)) <= 0 or
            'sqanwd' in model.model.yaml):
        raise RuntimeError('QPRR config or Detect head is invalid')
