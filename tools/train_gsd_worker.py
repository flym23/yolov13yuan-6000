#!/usr/bin/env python3
"""Train one deterministic GSD seed under the fixed URPC2020half protocol."""
...
from gsd_experiments import MODEL_FILES, STRUCTURES, resolve_model  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--stage", choices=tuple(stage for stage in MODEL_FILES if stage != "g0"), required=True)
...
def main() -> None:
    ...
    metadata = {"stage": args.stage, "structure": STRUCTURES[args.stage], "config": str(model_cfg), "pretrained": str(pretrained), "data": str(data), "seed": args.seed, "epochs": args.epochs, "patience": args.patience, "amp": False, "dataloader_workers": 2, "plots": False, "deterministic": True, "pin_memory": False, "started_at": datetime.now(timezone.utc).isoformat()}
    metadata_path.parent.mkdir(parents=True, exist_ok=True)
    metadata_path.write_text(json.dumps(metadata, ensure_ascii=False, indent=2), encoding="utf-8")
    model = YOLO(str(model_cfg))
    model.load(str(pretrained))
    model.train(data=str(data), imgsz=640, batch=16, epochs=args.epochs, optimizer="auto", lr0=0.01, lrf=0.01, close_mosaic=5, patience=args.patience, workers=2, device=0, amp=False, deterministic=True, plots=False, seed=args.seed, project=str(root / "runs" / "train"), name=args.name, exist_ok=False)
