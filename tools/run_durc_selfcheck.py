...
def main():
    ...
    incompatible = wrapped_block.load_state_dict(
        original_block.state_dict(), strict=False
    )
    assert not incompatible.unexpected_keys
    assert incompatible.missing_keys and all(
        key.startswith("drsc.") for key in incompatible.missing_keys
    )
    ...
    if device.type == "cuda":
        ...
        half_input = torch.randn(
            2, 16, 16, 16, device=device, dtype=torch.float16, requires_grad=True
        )
