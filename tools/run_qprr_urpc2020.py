#!/usr/bin/env python3
"""D1 -> D2 -> D3 full-URPC2020 chain, three managed seeds at each stage."""
...
def finalize(run: Path, rows: list[dict], statistics_rows: list[dict]) -> None:
    ...
    for candidate in MODELS:
        vs_a0 = comparisons[f'{candidate}_vs_A0']
        guardrails = dict(
            AP_S_drop_at_most_0_20pp=vs_a0['AP_S']['mean_delta'] >= -0.002,
            mAP50_95_drop_at_most_0_20pp=vs_a0['mAP50_95']['mean_delta'] >= -0.002,
            AP_M_not_lower_in_all_seeds=vs_a0['AP_M']['negative_seeds'] < 3,
            AP_L_not_lower_in_all_seeds=vs_a0['AP_L']['negative_seeds'] < 3,
        )
        modes = dict(
            precision=vs_a0['P']['mean_delta'] >= 0.003 and
                vs_a0['P']['positive_seeds'] >= 2 and vs_a0['F1']['mean_delta'] >= -0.0015,
            recall=vs_a0['R']['mean_delta'] >= 0.002 and
                vs_a0['R']['positive_seeds'] >= 2 and vs_a0['F1']['mean_delta'] >= -0.0015,
            balanced_F1=vs_a0['F1']['mean_delta'] > 0 and
                vs_a0['F1']['positive_seeds'] >= 2,
        )
        acceptance[candidate] = dict(modes=modes, guardrails=guardrails,
            accepted=any(modes.values()) and all(guardrails.values()))
    decision = dict(full_dataset_only=True, unconditional=True,
                    acceptance=acceptance, d0_equivalence_only=True)
...
def main() -> None:
    ...
    state = dict(old)
    state.update(run_id=run.name.removeprefix('qprr_urpc2020_'), run_root=str(run),
    ...
    try:
        ...
        if args.wait:
            ...
            lock.close()
            os.execv('/bin/bash', ['bash', str(ROOT / 'scripts/run_qprr_urpc2020.sh'),
        ...
        for reference in REFERENCE_STAGES:
            ...
            reference_rows = [enrich_reference(row, reference) for row in snapshot['seeds']]
        ...
        for stage in MODELS:
            for mode in ('train', 'test'):
                ...
                for seed in SEEDS:
                    ...
                    command = [sys.executable, '-u', str(ROOT / 'tools/train_qprr_urpc2020_worker.py'),
