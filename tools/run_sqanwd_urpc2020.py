#!/usr/bin/env python3
"""S2 -> S3 full-URPC2020 chain, three managed seeds at each stage."""
...
from tools.run_ucra_v2 import load, stop_processes, summarize, write_csv
from tools.sqanwd_urpc2020_common import (
    DATA, MODELS, REFERENCE_STAGES, SEEDS, SETTINGS, UPSTREAM_STATE,
    now, require_run_root, sha256, verify_contract, write_json,
)
...
def finalize(run: Path, rows: list[dict], statistics_rows: list[dict]) -> None:
    ...
    comparisons = {}
    for candidate, reference in (('S2', 'A0'), ('S3', 'A0'), ('S3', 'B1'), ('S3', 'S2')):
        comparison = {}
    ...
    acceptance = {}
    for candidate, reference in (('S2', 'A0'), ('S3', 'A0')):
        comparison = comparisons[f'{candidate}_vs_{reference}']
        checks = dict(
            mean_mAP50_95_positive=comparison['mAP50_95']['mean_delta'] > 0,
            positive_at_least_two_seeds=comparison['mAP50_95']['positive_seeds'] >= 2,
            AP_S_mean_drop_at_most_0_20pp=comparison['AP_S']['mean_delta'] >= -0.002,
            P_mean_drop_at_most_0_35pp=comparison['P']['mean_delta'] >= -0.0035,
            R_mean_drop_at_most_0_35pp=comparison['R']['mean_delta'] >= -0.0035,
            no_persistent_AP_M_decline=not (
                comparison['AP_M']['mean_delta'] < 0 and comparison['AP_M']['negative_seeds'] >= 2),
            no_persistent_AP_L_decline=not (
                comparison['AP_L']['mean_delta'] < 0 and comparison['AP_L']['negative_seeds'] >= 2),
        )
        acceptance[candidate] = dict(checks=checks, accepted=all(checks.values()))

    decision = dict(full_dataset_only=True, s3_unconditional=True,
                    acceptance=acceptance, phase2_executed=False)
...
def main() -> None:
    ...
    state = dict(old)
    state.update(run_id=run.name.removeprefix('sqanwd_urpc2020_'), run_root=str(run),
    ...
    try:
        ...
        if args.wait:
            ...
            lock.close()
            os.execv('/bin/bash', ['bash', str(ROOT / 'scripts/run_sqanwd_urpc2020.sh'),
        ...
        for reference in REFERENCE_STAGES:
            ...
            mean_rows.extend(dict(stage=reference, metric=metric, **values)
                             for metric, values in snapshot['statistics'].items())
        for stage in MODELS:
            for mode in ('train', 'test'):
                ...
                for seed in SEEDS:
                    ...
                    command = [sys.executable, '-u', str(ROOT / 'tools/train_sqanwd_urpc2020_worker.py'),
            ...
            write_json(run / 'test/AllSeed_summary.json', dict(data=str(DATA),
                ...
                full_dataset_only=True, s3_unconditional=True, std_ddof=1))
