...
def make_image_link(source: Path, destination: Path) -> None:
    """Hard-link source images so path resolution cannot fall back to shared labels."""
    if destination.is_symlink():
        ...
        raise RuntimeError(
            f"refusing to replace non-directory image path: {destination}"
        )
    ...
    for source_path in sorted(source.rglob("*")):
        ...
        if destination_path.exists():
            if destination_path.samefile(source_path):
                continue
            raise RuntimeError(
                f"refusing to replace unrelated image file: {destination_path}"
            )
...
def build_durc_dataset(source_data: Path, target_root: Path, target_data: Path) -> dict:
    ...
    for split in ("train", "val", "test"):
        ...
        cache_path = destination_root.with_suffix(".cache")
        cache_path.unlink(missing_ok=True)
        cache_path.with_suffix(".cache.npy").unlink(missing_ok=True)
