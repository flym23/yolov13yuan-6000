#!/usr/bin/env python3
"""Audit original YOLOv13 or GSD-G4 checkpoint migration into full GMR-YOLOv13."""
...
ALLOWED_NEW_PREFIXES = ("model.4.detail_router.", "model.4.geometry.", "model.15.", "model.32.p3_reconciler.")
...
def main() -> None:
    ...
    target_cfg = root / "ultralytics" / "cfg" / "models" / "v13" / "yolov13-gmr-r7-full.yaml"
    ...
    # Original YOLOv13 uses parameter-free nearest upsampling at layer 15; a GSD-G4 source has MCAS keys there.
    required = REQUIRED_PREFIXES + (("model.15.",) if any(key.startswith("model.15.") for key in source) else ())
    required += (("model.32.cv3.",) if source_nc == target_nc else ())
    ...
    print(f"GMR checkpoint migration verification passed: {args.output}")
