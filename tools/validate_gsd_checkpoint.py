#!/usr/bin/env python3
"""Audit original YOLOv13 checkpoint migration into full GSD-YOLOv13."""
...
def main() -> None:
    ...
    target_cfg = root / "ultralytics" / "cfg" / "models" / "v13" / "yolov13-gsd-t7-full.yaml"
    ...
    report = {"source_weights": str(weights), "target_config": str(target_cfg), "source_nc": source_nc, "target_nc": target_nc, "classification_tower_reinitialized": source_nc != target_nc, "source_keys": len(source), "target_keys": len(target), "matched_keys": len(matched), "missing_required_prefixes": missing_required, "unexpected_unmatched_target_keys": unexpected}
    args.output.parent.mkdir(parents=True, exist_ok=True)
    args.output.write_text(json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8")
    if missing_required or unexpected:
        raise RuntimeError(json.dumps(report, ensure_ascii=False, indent=2))
    print(f"GSD checkpoint migration verification passed: {args.output}")
