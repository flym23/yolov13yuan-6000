#!/usr/bin/env python3
"""Aggregate completed GSD test summaries; G0 remains the two designated reused baselines."""
...
from tools.gsd_experiments import REFERENCE_SUMMARIES, STRUCTURES, TRAIN_STAGES  # noqa: E402
...
def read_references() -> dict[str, object]:
    ...
    for label, raw_path in REFERENCE_SUMMARIES.items():
        ...
        if path.is_file():
            references[label]["summary"] = json.loads(path.read_text(encoding="utf-8"))
    return references
...
def main() -> None:
    ...
    for stage in args.stages:
        for seed in (0, 1, 2):
            ...
            payload = json.loads((root / "runs" / "test" / name / "summary_metrics.json").read_text(encoding="utf-8"))
            row = {"stage": stage, "seed": seed, "test_name": name, "structure": STRUCTURES[stage], "weights": payload["weights"], "Params": int(payload["model"].get("parameters", 0)), "GFLOPs": float(payload["model"].get("gflops", 0.0))}
    ...
    summary: dict[str, object] = {"run_id": args.run_id, "dataset": "/home/room305/ZZF/URPC2020half/data.yaml", "settings": {"epochs": 300, "patience": 40, "amp": False, "workers": 2, "plots": False, "pin_memory": False}, "g0_reused_baselines": read_references(), "stages": {}}
    for stage in args.stages:
        rows = [row for row in records if row["stage"] == stage]
        stage_summary = {"structure": STRUCTURES[stage], "n": len(rows), "runs": rows, "metrics_percent": {label: stats([float(row[label]) for row in rows]) for label in (*METRICS, *SCALE_METRICS)}, "Params": rows[0]["Params"], "GFLOPs": rows[0]["GFLOPs"]}
