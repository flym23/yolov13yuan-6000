...
def verify_project_ultralytics() -> None:
    """Fail closed if an absolute worker path imported site-packages Ultralytics."""
    package_file = Path(ultralytics.__file__).resolve()
    if ROOT_DIR.resolve() not in package_file.parents:
        raise RuntimeError(
            f"Refusing to train with a non-project Ultralytics package: {package_file}; "
            f"expected it below {ROOT_DIR.resolve()}"
        )
