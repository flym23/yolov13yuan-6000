#!/usr/bin/env python3
"""Audit baseline checkpoint migration into the primary CPCR L4 model."""
...
REQUIRED_PREFIXES = ("model.32.cv2.", "model.32.cv3.", "model.32.dfl.")
ALLOWED_NEW_PREFIXES = ("model.32.recovery.",)
...
def main() -> None:
    ...
    target_cfg = root / "ultralytics" / "cfg" / "models" / "v13" / "yolov13n-cpcr-l4-full.yaml"
    ...
    required = tuple(prefix for prefix in REQUIRED_PREFIXES if prefix != "model.32.cv3." or source_nc == target_nc)
    ...
    report = {"source_weights": str(weights), "target_config": str(target_cfg), "source_nc": source_nc, "target_nc": target_nc,
    ...
    print(f"CPCR checkpoint migration verification passed: {args.output}")
